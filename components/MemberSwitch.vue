<template>
  <div class="member-switch">
    ...
    <h1 class="h1" style="margin: 0">Team Members</h1>
    <div class="carousel">
      <div class="previous prevnext" @click="changeCarousel(-1)">
        <img src="@/assets/icons/arrow-back.svg" alt="previous member" />
      </div>
      <div class="next prevnext" @click="changeCarousel(1)">
        <img src="@/assets/icons/arrow-forward.svg" alt="next member" />
      </div>
      <div v-for="m in members" :key="m.id" class="image-wrapper">
        <img
          class="polygon"
...
export default {
  ...
  computed: {
    wrappingIndex: function() {
      return (
        ((this.totalChanges % this.members.length) + this.members.length) %
        this.members.length
      );
    },
